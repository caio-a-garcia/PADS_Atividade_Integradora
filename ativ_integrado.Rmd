...
```{r}
# Transformando as variáveis em categorias
...
```{r}
fit <- rpart(fechado ~ ., method = "class", data = treinamento)
...
```
>>>>>>> f08de94d8b2c9d3c7e228d8b8392ab43e7321ca0
Random Forest
...
rf <- ranger(fechado ~ ., mtry = 4, num.trees = 350, probability = TRUE, data = treinamento)
...
# Classificar as previsões com base no limiar
predicoes <- ifelse(prob_fechado > .18, 1, 0)

# Gerar matriz de confusão
matriz_confusao <- table(observado = teste$fechado, predito = predicoes)

# Calcular taxa de erro
erro_calculado <- sum(teste$fechado != predicoes) / length(teste$fechado)

# Matriz e erro
list(matriz_confusao, erro_calculado)
```
``` {r}
desempenho <- roc(teste$fechado, prob_fechado)

coords(desempenho, 0.18, ret = c("1-accuracy", "sensitivity", "specificity", "ppv", "npv"))
...
### VERIFICARRRRRR
arvore <- rpart (fechado ~. , data = treinamento)

pred_arvore <- predict(arvore, teste)

tibble(modelo = "floresta aleatória", 
       probabilidade = predict_arvore[,2],
       classe = teste$fechado) %>% 
  bind_rows(tibble(modelo = "floreste aleatória",
                   probabilidade = predict(rf, teste, type = "response")$predictions[,2],
                   classe = teste$fechado)) %>% 
  group_by(modelo) %>% 
...
```{r}
# Salvando resultados:
resultados_floresta$accuracy[resultados_logist$modelo == "primeiro"] <- accuracy
resultados_logist$AUC[resultados_logist$modelo == "primeiro"] <- auc_value
resultados_logist$sensitivity[resultados_logist$modelo == "primeiro"] <- sensitivity_value
resultados_logist$specificity[resultados_logist$modelo == "primeiro"] <- specificity_value
```
